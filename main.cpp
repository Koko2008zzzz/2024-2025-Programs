...
//Motors for Sub-Assmeblies
 ...
 pros::Motor bucket(11,pros::MotorGear::red);

//Sensors
 pros::Rotation bucketSensor(-12);

//Pneumatics
 pros::adi::DigitalOut mogoMech('a', false);

//on/off switch for PD
bool enableBucketPD = false;
...
int bucketPD () {
	while (enableBucketPD) {

		error = bucketSensor.get_angle() - targetAngle;
		derivative = error - prevError;

		double motorVoltage = (error * kP + derivative * kD) / 12; //change it -12 if spins reverse

		bucket.move(motorVoltage);

		prevError = error;

		//Optional can be commented out
		if (bucketSensor.get_angle() >= targetAngle -1 && bucketSensor.get_angle() <= targetAngle +1) {
			bucket.brake();
		}
...
void initialize() {
	...
	//sets brake modes
	bucket.set_brake_mode(pros::E_MOTOR_BRAKE_HOLD);
...
void autonomous() {
	pros::Task FourBarPID (bucketPD); //starts Four-Bar PD loop
